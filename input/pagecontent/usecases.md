...
*   **交換流程**：
    ...
    2. 醫院端系統生成符合 `OHE-Bundle` (type=document) 的健檢結果，其中以 `OHE-Composition` 封裝基本資料、理學檢查與一般實驗室檢驗（Hb, WBC, 尿蛋白, 尿潛血）。
    3. 醫院系統透過安全 API 將結果傳輸至事業單位（雇主公司）之職安衛系統或職場健康服務管理平台。
*   **優勢**：取代紙本郵寄與手動登打，縮短錄取報到審查流程。
*   **實體資料範例與架構說明**：
    本指引提供了一個完整的實體資料交換範例 [健檢紀錄完整封裝 Bundle 範例 (example-complete-bundle)](Bundle-example-complete-bundle.html)，其內含一個臨床文件 `OHECompositionProfile` (一般健檢報告) 以及對應附表九法定項目的所有實體 `Observation` 資源，包括：
...
*   **交換流程**：
    - 噪音作業：封裝純音聽力圖數據（`OHE-HearingTest`）。
    - 鉛作業：封裝血中鉛濃度（`OHE-LabResult-Special`）與紅血球、比容值等指標。
    - 粉塵作業：封裝胸部 X 光報告（`OHE-ImagingStudy` / `OHE-DiagnosticReport`）與肺功能數值（FEV1, FVC）。
...
*   **交換流程**：
    ...
    2. 填寫臨場紀錄並產出 `OHE-Composition-ServiceRecord`（包含 `OHE-Encounter-Service` 服務事件與 `OHE-Procedure-ServiceActivity` 辦理事項）。
...
*   **資料表現**：
    - 在 `OHE-ClinicalImpression` 中以 `ext-health-mgmt-level` 記錄判定分級，於 `summary` 記錄醫師意見。
...
*   **交換流程**：
    - 當分級為 `level-4`：職安衛系統自動生成 `OHE-CarePlan`（適性配工計畫），其中包含 `ext-fitness-for-work`（工作調整建議：變更場所、工作或縮短工時）並由雇主執行。
    - 當分級為 `level-3`：醫事系統自動生成 `OHE-ServiceRequest`（追蹤檢查要求），排程三個月內重檢或現場評估。
...
*   **交換流程**：
    - 健檢機構與事業單位系統透過 API 呼叫，傳輸符合本指引標準的 `OHE-Bundle` (含 `ext-labor-report-code` 通報代碼) 至政府通報伺服器，由伺服器自動解析、存檔並產出統計報表。
